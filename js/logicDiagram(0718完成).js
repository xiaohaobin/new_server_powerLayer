...
try {
	...
	//播放器的图表
	function CurpowerChart(mainData) {
		//配置
		var chart_confing = {
			credits: { //禁用版权信息
				enabled: false
	...
	//全部数据一次加载图表
	function backChartArr(arr) {
		var _arr = [];
		for (var i = 0, len = arr.length; i < len; i++) {
			_arr.push(arr[i].panelPower);
		}
		return _arr;
	}

	//模拟数据（2015-04-15一整天的数据）
	...
	$(document).ready(function() {
		...
		function draw(nPos, recalculateScreen, initial) {
			...
			if(nPos > 0) treeUpdateDataFn(site_data);
	...
	var tree = d3.layout.tree()
		.size(
			[getTreeLayoutHR(toTreelayoutData(site_data),tree_height), tree_width]	
		);
	...
	// 根节点和位置
	log_root = toTreelayoutData(site_data);//主要数据转化赋值
	...
	//逆变器数量
	$(".siteStatic_inv_num").text(site_data.inverter.length);
	//优化器数量
	$(".siteStatic_panel_num").text(oComFn.backSiteDataCf(site_data).length)
...
//			.remove();
	...
	function treeUpdateDataFn(data){			
		...
		var trans_tree_data = toTreelayoutData(site_data);//适配树状布局的数据
